...
cli_alert_info("Loading results file {.file longrun.rds}")
d   <- readRDS('longrun.rds')
cli_alert_success("Finished")

cli_alert_info("Loading configuration file {.file config_longrun.rds}")
d_c <- readRDS('config_longrun.rds')
cli_alert_success("Finished")
...
pdf('plots-all-may24.pdf', width=11, height=8.5)
renderPlots2(sp)
dev.off()
