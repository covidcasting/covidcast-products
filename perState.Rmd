  ...
  purrr::pmap(results, function(state, ...) {
    ...
    plot <- highchart(type = 'stock') %>% 
      ...
      hc_add_series(
        ...
        #   list(data$infections.lo, data$infections.hi)
        # ) %>% xts(),
        data = as.xts(
          data.frame(lo = data$infections.lo, hi = data$infections.hi)
        ),
        type = 'errorbar', yAxis = 2
      ) %>%
...
state.charts <- makeCharts(results)
