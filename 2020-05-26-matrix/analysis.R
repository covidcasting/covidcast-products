...
path_result <- "source-smoothing-matrix.rds"
path_config <- "config_source-smoothing-matrix.rds"
...
d_j <- left_join(d_c, d, by = c('state', 'smoothing', 'source'))
...
simpler <- select(final, state, smoothing, source, result = result.result)
...
) %>% arrange(state, source, smoothing) -> sp
cli_alert_success("Finished")
...
renderPlots2 <- function(sp) {
  pwalk(
    sp,
    function(state_name, source, smoothing,
             result, plots, RtNaiveEstim, RtEstim) {

      cli_alert_info("Producing plots for region {.code {state_name}-{source}-{smoothing}}")

      x_start <- as.Date('2020/01/20')
      x_end   <- as.Date('2020/05/24')
      ...
      allPlots <- gridExtra::grid.arrange(
        ...
        nrow = 4,
        ncol = 1,
        top = glue("{state_name}-{source}-{smoothing}")
      )
...
pdf('plots-matrix.pdf', width=11, height=8.5)
renderPlots2(sp)
dev.off()
